...
class SabioNoSQL(data_source.HttpDataSource):
    ...
    def __init__(self, name=None, cache_dirname=None, clear_content=False, load_content=False, max_entries=float('inf'),
        ...
        self.collection = self.db['sabio_rk']
    ...
    def load_content(self):
        ...
        new_ids = list(set(ids).difference(
            set(l.id for l in self.session.query(KineticLaw).all())))
        new_ids.sort()
        ...
        ##################################
        # download compounds
        compounds = self.session.query(Compound).filter(
            ~Compound.structures.any()).order_by(Compound.id).all()
        ...
        loaded_new_ids = list(set(new_ids).intersection(
            set(l.id for l in self.session.query(KineticLaw).all())))
        loaded_new_ids.sort()
        ...
        compound_structures = self.session.query(CompoundStructure).filter_by(
            _value_inchi_formula_connectivity=None).all()

        if self.verbose:
            print('Calculating searchable structures for {} structures ...'.format(
                len(compound_structures)))
    ...
    def load_kinetic_laws(self, ids):
        ...
        loaded_ids = [l.id for l in self.session.query(
            KineticLaw).order_by(KineticLaw.id)]
        ...
        if type == 'SPC':
            ...
            for cross_reference in cross_references:
                if cross_reference.namespace == 'uniprot':
                    specie.subunits.append(EnzymeSubunit(
    ...
    def create_kinetic_law_from_sbml(self, id, sbml, specie_properties, functions, units):
        ...
        # ID
        annotated_id = next((int(float(x_ref.id))
                             for x_ref in x_refs if x_ref.namespace == 'sabiork.kineticrecord'), None)
    ...
    def normalize_kinetic_laws(self, ids):
        ...
        for i_law, law in enumerate(self.session.query(KineticLaw).filter(KineticLaw.id.in_(ids)).all()):
            if self.verbose and (i_law % 100 == 0):
                print('  Normalizing kinetic law {} of {}'.format(
                    i_law + 1, len(ids)))
    ...
    def normalize_parameter_value(self, name, type, value, error, units, enzyme_molecular_weight):
        ...
        raise ValueError(
            'Unsupported units "{}" for parameter type {}'.format(units, type_name))
        ...
        for i_child in range(xml.getNumChildren()):
            ...
            for i_li in range(bag.getNumChildren()):
                ...
                for namespace, id in resources:
                    query = self.session.query(Resource).filter_by(
    ...
    def get_specie_reference_from_sbml(self, specie_id):
        ...
        if q.count() != 1:
            raise ValueError(
                'Could not find species with id {}'.format(specie_id))
        specie = q.first()

        if compartment_name != 'Cell':
            q = self.session.query(Compartment).filter_by(
                name=compartment_name)
            if q.count() != 1:
                raise ValueError(
                    'Could not find compartment with name "{}"'.format(compartment_name))
            compartment = q.first()
        else:
            compartment = None
    ...
    def get_parameter_by_properties(self, kinetic_law, parameter_properties):
        ...
        def func(parameter):
            return (parameter.compound is None and parameter_properties['associatedSpecies'] is None) or \
                (parameter.compound is not None and parameter.compound.name ==
                 parameter_properties['associatedSpecies'])
        parameters = list(filter(func, kinetic_law.parameters))
        if len(parameters) == 1:
            return parameters[0]
