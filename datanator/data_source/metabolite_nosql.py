...
class MetaboliteNoSQL():
	def __init__(self, verbose, dababase):
		...
		if self.database == 'ecmdb':
			...
			self.compound_index = self.domain + '/download/ecmdb.json.zip' # list of metabolites
			self.compound_url = self.domain + '/compounds/{}.xml'
			self.collection_dir = './cache/ecmdb/'
		else:
			self.domain = 'http://ymdb.ca'
			self.compound_index = self.domain + '/system/downloads/current/ymdb.json.zip' # list of metabolites
			self.compound_url = self.domain + '/compounds/{}.xml'
	...
	def write_to_json(self):
		...
		for i_entry, entry in enumerate(entries):
			...
			if self.database == 'ecmdb':
				response = requests.get(self.compound_url.format(entry['m2m_id']))
				try:
					response.raise_for_status()
			...
			if self.database =='ecmdb':
				file_name = os.path.join(self.collection_dir + entry['m2m_id'] + '.json')
			else:
